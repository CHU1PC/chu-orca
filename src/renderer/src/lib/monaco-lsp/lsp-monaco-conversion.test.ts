...
describe('lspDiagnosticsToMonacoMarkers', () => {
  ...
  it('uses a separate owner and display source for each server', () => {
    expect(lspMarkerOwner('pyright')).toBe('orca-lsp:pyright')
    expect(lspMarkerOwner('ruff')).not.toBe(lspMarkerOwner('pyright'))
    expect(
      lspDiagnosticsToMonacoMarkers(
        [{ range: lspRange, message: 'unused import' }],
        severities,
        'ruff'
      )[0].source
    ).toBe('Ruff')
