...
import { resolveTokenScope } from './textmate-scope-resolver'
import type { ThemeScopeIndex } from './textmate-scope-resolver'
...
export type TextMateTokensProviderOptions = {
  ...
  themeScopeIndex: ThemeScopeIndex
    ...
    tokenize(line, state) {
      ...
      return {
        ...
        tokens: result.tokens.map((token) => ({
          ...
          scopes: resolveTokenScope(token.scopes, themeScopeIndex) || fallbackScopeName
        }))
      }
    }
  }
...
export async function createTextMateTokensProvider(
  ...
  return createTokensProvider(grammar, options.scopeName, options.themeScopeIndex)
}
